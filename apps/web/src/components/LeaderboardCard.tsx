...
const rankColors = [
  ...
  'from-slate-400/20 to-gray-500/10 border-slate-400/30',
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`font-semibold text-sm truncate ${isCurrentUser ? 'text-brand-300' : 'text-white'}`}>
          {entry.name} {isCurrentUser && <span className="text-brand-400 text-xs">(You)</span>}
        </p>
        <p className="text-white/40 text-xs mt-0.5">{entry.completions} completions</p>
      </div>

      {/* XP */}
      <div className="text-right flex-shrink-0">
        <p className={`font-display font-bold ${isTop3 ? 'text-gradient-gold text-lg' : 'text-white text-sm'}`}>
